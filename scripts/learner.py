...
class Trainer(object):
    ...
    def fit(self, num_epochs, args, device='cuda:0'):
        ...
        for epoch in pbar:
            ...
            self.early_stop(f1_score(y_true, y_pred, average="macro"), self.model)
    ...
    def predict(self, device='cuda:0', pbar=None):
        ...
        preds_dict = {
            'y_true': np.zeros([current_size, 8]),
            'y_pred': np.zeros([current_size, 8])
        }
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(progress_bar(self.val_data_loader, parent=pbar, leave=(pbar is not None))):
                loss, num_rows, y_pred, targets,sentences_embeddings = self.model(batch, device)
...
class EvaluateOnTest(object):
    ...
    def predict(self, device='cuda:0', pbar=None):
        ...
        start_time = time.time()
        sentences_embeddings_all = []
        with torch.no_grad():
            index_dict = 0
            for step, batch in enumerate(progress_bar(self.test_data_loader, parent=pbar, leave=(pbar is not None))):
                _, num_rows, y_pred, targets,sentences_embeddings = self.model(batch, device)
                sentences_embeddings_all.extend(sentences_embeddings.tolist())
                current_index = index_dict
                preds_dict['y_true'][current_index: current_index + num_rows, :] = targets
                preds_dict['y_pred'][current_index: current_index + num_rows, :] = y_pred
                index_dict += num_rows
        ...
        g.close()
        with open(self.model_path+'.josn','w') as f:
            json.dump(sentences_embeddings_all,f)