...
Options:
    ...
    --beta-loss=<float>              weight used to balance the loss [default: 0.2]
    --gamma-loss=<float>              weight used to balance the loss [default: 0.2]
    --temperature=<float>              weight used to balance the loss [default: 0.2]
"""
...
filename = str(args['--seed'])
fw = open('configs/' + filename + '.json', 'a')
model_path = str(args['--seed']) + '.pt'
...
model = SpanEmo(output_dropout=float(args['--output-dropout']),
                ...
                beta=float(args['--beta-loss']),
                gamma=float(args['--gamma-loss']),
                temperature1=float(args['--temperature1']),
                temperature2=float(args['--temperature2']))
#############################################################################
# Start Training
#############################################################################
